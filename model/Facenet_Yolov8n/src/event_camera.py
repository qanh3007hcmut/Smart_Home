            ...
            while True:
                ...
                if "motion" in self.detect_types:
                    ...
                    if current_time - self.detected >= self.detected_interval:
                        try: client.publish("home/kitchen/people/state", "1" if motion_detected else "0", qos=1, retain=True)
                        except: pass
                        self.detected = time.time()
                ...
                try: client.publish("home/living/people/fall/state", "1" if fall_detected else "0", qos=1, retain=True)
                except: pass
